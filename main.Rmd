...
# 
# start_date <- as.Date("2009-01-01") # select start date
...
# 
# write.csv(df_discharge, "data_rhone/Tables/discharge_rhone_2009-2019.csv", row.names = F)
...
# # read discharge data
# df_Q <- read.csv("data_rhone/Tables/discharge_rhone_2009-2019.csv")
# df_Q$yyyy.mm.dd <- ymd(df_Q$yyyy.mm.dd)
# colnames(df_Q) <- c("yyyy/mm/dd", 1:67)
# 
# # Daily discharge by hydrological cycle - start September 21
# dfq_hy_2009 <- df_Q[df_Q$`yyyy/mm/dd` < '2009-09-21',]
# dfq_hy_2010 <- df_Q[df_Q$`yyyy/mm/dd` >= '2009-09-21' & df_Q$`yyyy/mm/dd` < '2010-09-21',]
# dfq_hy_2011 <- df_Q[df_Q$`yyyy/mm/dd` >= '2010-09-21' & df_Q$`yyyy/mm/dd` < '2011-09-21',]
# dfq_hy_2012 <- df_Q[df_Q$`yyyy/mm/dd` >= '2011-09-21' & df_Q$`yyyy/mm/dd` < '2012-09-21',]
# dfq_hy_2013 <- df_Q[df_Q$`yyyy/mm/dd` >= '2012-09-21' & df_Q$`yyyy/mm/dd` < '2013-09-21',]
# dfq_hy_2014 <- df_Q[df_Q$`yyyy/mm/dd` >= '2013-09-21' & df_Q$`yyyy/mm/dd` < '2014-09-21',]
# dfq_hy_2015 <- df_Q[df_Q$`yyyy/mm/dd` >= '2014-09-21' & df_Q$`yyyy/mm/dd` < '2015-09-21',]
# dfq_hy_2016 <- df_Q[df_Q$`yyyy/mm/dd` >= '2015-09-21' & df_Q$`yyyy/mm/dd` < '2016-09-21',]
# dfq_hy_2017 <- df_Q[df_Q$`yyyy/mm/dd` >= '2016-09-21' & df_Q$`yyyy/mm/dd` < '2017-09-21',]
# dfq_hy_2018 <- df_Q[df_Q$`yyyy/mm/dd` >= '2017-09-21' & df_Q$`yyyy/mm/dd` < '2018-09-21',]
# dfq_hy_2019 <- df_Q[df_Q$`yyyy/mm/dd` >= '2018-09-21' & df_Q$`yyyy/mm/dd` < '2019-09-21',]
# 
# write.csv(dfq_hy_2009, 'data_rhone/Tables/discharge_hy/dfq_hy_2009.csv', row.names = F)
# write.csv(dfq_hy_2010, 'data_rhone/Tables/discharge_hy/dfq_hy_2010.csv', row.names = F)
# write.csv(dfq_hy_2011, 'data_rhone/Tables/discharge_hy/dfq_hy_2011.csv', row.names = F)
# write.csv(dfq_hy_2012, 'data_rhone/Tables/discharge_hy/dfq_hy_2012.csv', row.names = F)
# write.csv(dfq_hy_2013, 'data_rhone/Tables/discharge_hy/dfq_hy_2013.csv', row.names = F)
# write.csv(dfq_hy_2014, 'data_rhone/Tables/discharge_hy/dfq_hy_2014.csv', row.names = F)
# write.csv(dfq_hy_2015, 'data_rhone/Tables/discharge_hy/dfq_hy_2015.csv', row.names = F)
# write.csv(dfq_hy_2016, 'data_rhone/Tables/discharge_hy/dfq_hy_2016.csv', row.names = F)
# write.csv(dfq_hy_2017, 'data_rhone/Tables/discharge_hy/dfq_hy_2017.csv', row.names = F)
# write.csv(dfq_hy_2018, 'data_rhone/Tables/discharge_hy/dfq_hy_2018.csv', row.names = F)
# write.csv(dfq_hy_2019, 'data_rhone/Tables/discharge_hy/dfq_hy_2019.csv', row.names = F)
...
```{r q, fig.cap= 'Manning’s coefficient of each river reach along the Rhône River.', fig.height=4, fig.width=7, fig.dpi=500, echo=FALSE, warning=FALSE, include=FALSE}

# Discharge main Rhone River
df_discharge <- read.csv("data_rhone/Tables/discharge_rhone_2009-2019.csv")
...
ggplot(df_q_long) +
  geom_line(aes(reach_ID, runoff, colour = Stat), lwd = 0.5) +
  facet_wrap(~HY)
...
#     scale_x_continuous(n.breaks = 10) +
#     scale_fill_manual(breaks = c('blue4', 'coral4', 'black', 'blue'),
#                       values = c('blue4', 'coral4', 'black', 'blue'),
#                       labels = c('Discharge', 'Altitude', 'Dams', 'Tributaries'), name = '') +
#     scale_colour_manual(breaks = c('blue4', 'coral4', 'black', 'blue'),
#                         values = c('blue4', 'coral4', 'black', 'blue'),
#                         labels = c('Discharge', 'Altitude', 'Dams', 'Tributaries'), name = '') +
#   labs(x = 'Reach ID') +
#   theme_article(base_family = "Times New Roman") +
...
# ggsave(filename = 'img/discharge.svg',, plot = p_q, width = 16, height = 8, units = 'cm', dpi = 300)

```


## Trap efficiency (GILL, 1979)


Trap efficiency was estimated by the ratio of reservoir capacity (*C*) to catchment area (*A*). This relationship was proposed by Brown (1944), who presented the following Equation (1):

$$
E = 1 - \frac{1}{1 + k \cdot \frac{C}{A}} \tag{1}
$$
...
Equation (2) presents the same relationship using metric units:

$$
E = 1 - \frac{1}{1 + 0.0021 \cdot k \cdot \frac{C}{A}} \tag{2}
$$
...
Therefore, in the Rhône catchment, Equation (2) was applied to estimate the trap efficiency for the dams located along the main river.

**Table 1** presents the trap efficiency (*E*) values for fine, medium, and coarse sediments at each dam on the Rhône River.



**Table 1 – Trap efficiency (E) for fine, medium, and coarse sediments at each dam located on the Rhône River, including values estimated by similarity.**

| Dam                 | C (m³)     | A (km²) | Fine (< 0.062 mm) | Medium (0.062–2 mm) | Coarse (> 2 mm) |
|---------------------|------------|---------|--------------------|----------------------|-----------------|
| Verbois             |            |         | 0.12               | 0.22                 | 0.74            |
| Chancy Pougny       | 14000000   | 10294   | 0.12               | 0.22                 | 0.74            |
| Génissiat           | 72000000   | 10910   | 0.39               | 0.58                 | 0.93            |
| Seyssel             | 7600000    | 11300   | 0.06               | 0.12                 | 0.59            |
| Chautagne           |            | 12700   | 0.21               | 0.37                 | 0.85            |
| Belley              |            | 12985   | 0.21               | 0.37                 | 0.85            |
| Brégnier-Cordon     |            | 14000   | 0.21               | 0.37                 | 0.85            |
| Sault-Brénaz        |            | 15380   | 0.21               | 0.37                 | 0.85            |
| Jons                | 55000000   | 20000   | 0.21               | 0.37                 | 0.85            |
| Pierre Bénite       |            | 50400   | 0.21               | 0.37                 | 0.85            |
| Vaugris             | 41000000   | 51700   | 0.07               | 0.14                 | 0.62            |
| Péage de Roussillon |            | 52880   | 0.21               | 0.37                 | 0.85            |
| Saint Valiier       |            | 53850   | 0.21               | 0.37                 | 0.85            |
| Bourg-lès Valences  |            | 66450   | 0.21               | 0.37                 | 0.85            |
| Beauchastel         |            | 66700   | 0.21               | 0.37                 | 0.85            |
| Baix-le Logis Neuf  |            | 69960   | 0.21               | 0.37                 | 0.85            |
| Montélimar          |            | 70900   | 0.21               | 0.37                 | 0.85            |
| Donzère-Mondragon   | 27000000   | 71000   | 0.04               | 0.07                 | 0.44            |
| Caderousse          | 123000     |         | 0.07               | 0.14                 | 0.62            |
| Avignon             |            |         | 0.04               | 0.07                 | 0.44            |

**Where:**  
- `C` is the reservoir capacity (m³)  
...
Preliminary results were obtained using three different sediment transport equations, each simulated under three distinct trap efficiency scenarios. The equations used to estimate sediment transport were developed by Wilkock and Crowe, Eugelund and Hansen, and, Ackers and White.

For each equation, three trap efficiency scenarios were considered: (i) no dams influence, where the model assumes no dams present along the river; (ii) 100% trap efficiency, where all the sediment reaching a dammed reach is fully retained; and (iii) variable trap efficiencies as presented in Table 1. Simulations were caried out for the period from 2010 to 2019, with annual results available in supplementary material.

Three grain size classes were used based on Krumbein's scale: sand (0.06-2 mm), gravel (2-256 mm), and boulder (>256 mm).

The following figures (from Figure \@ref(fig:TrE0eW) to Figure \@ref(fig:budgetE3eA)) present the cumulative bedload transported, deposited and sediment budget over the 10-year period for each equation and trap efficiency scenario.
...
```{r TrE2eA, echo=FALSE, fig.cap='Cumulative discharge and bedload deposited for sand (0.63-2 mm), gravel (2-256 mm), boulders (>256 mm), and the total of all three classes for each river reach along the Rhône River, calculated using the Ackers and White equation without accounting dams trap efficiency.'}
