...
map50 <- mapview(shp_gsd_approx, zcol = "D50", title("none"))
map_gsd <- mapview(shp_gsd)
...
valores <- shp_gsd_approx$D50
pal <- colorNumeric("YlGnBu", domain = valores)

leaflet(shp_gsd_approx) %>%
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(fillColor = ~pal(D50),
              color = "#444444",
              weight = 0.7,
              fillOpacity = 0.8) %>%
